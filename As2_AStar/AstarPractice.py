...
def a_astar(graph,start,goal,heuristic):
    ...
    while openlist:
        ...
        curr_f_score,current_node = heapq.heappop(openlist)
        ...
        for neighbour in graph[current_node]:
            ...
            tentative_g_score = g_score[current_node]+graph[current_node][neighbour]

            if tentative_g_score < g_score[neighbour]:
                g_score[neighbour]=tentative_g_score
                f_score[neighbour]= g_score[neighbour]+ heuristic[neighbour]

                heapq.heappush(openlist,(f_score[neighbour],neighbour))
...
def heuristic(node,goal):
    ...
    h= euclidean_distance(coordinates[node],coordinates[goal])
    print(f"heuristic of {node}: {h}")
    return h'''
heuristic = {
        'A': 11,
        'B': 6,
...
path = a_astar(Graph_nodes,'A', 'G',heuristic)
